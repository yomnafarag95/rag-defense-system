...
class BenchmarkRunner:
    ...
    def _plot_score_distribution(
        ...
        Plot a normalised risk-score distribution for benign vs attack

        samples and draw a vertical block-threshold line.
 
        The figure is saved to ``self.plots_dir / fig3_score_dist_<name>.png``

        and its path is returned so callers can embed it in a report.
        ...
        entries : list[dict] | None
            ...
            Result entries from ``self.results_log``.  When *None* the full

            ``self.results_log`` is used.

        block_threshold : float

            Risk-score value at which the pipeline switches from MONITOR to

            BLOCK.  Drawn as a dashed vertical line.

        name : str

            Label appended to the output filename for disambiguation.

        """

        if entries is None:

            entries = self.results_log
 
        benign_scores = [

            e["risk_score"] for e in entries if e["true_label"] == 0

        ]

        attack_scores = [

            e["risk_score"] for e in entries if e["true_label"] == 1

        ]
 
        if not benign_scores and not attack_scores:

            logger.warning(

                "_plot_score_distribution: no entries to plot (results_log empty)."
        ...
        ax.set_xlabel("Normalised Risk Score", fontsize=11)

        ax.set_ylabel("Density", fontsize=11)

        ax.set_title(

            "Fig. 3 — Risk Score Distribution (Benign vs. Attack)",
        ...
        out_path  = self.plots_dir / f"fig3_score_dist_{safe_name}.png"
    ...
    def run_all(self) -> dict:
        ...
        print("\n[eval] Generating Fig. 3 — risk score distribution ...")
